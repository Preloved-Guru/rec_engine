...
func (m *Master) getRecommend(request *restful.Request, response *restful.Response) {
	...
	case "image_based":
		if m.Config.Recommend.ImageEmbeddings.EnableImageRecommend {
			results, err = m.Recommend(ctx, response, userId, categories, n, m.RecommendImageBased)
		} else {
			server.BadRequest(response, fmt.Errorf("image-based recommendation is not enabled"))
			return
		}
	case "_":
		...
		for _, recommender := range m.Config.Recommend.Online.FallbackRecommend {
			...
			case "image_based":
				if m.Config.Recommend.ImageEmbeddings.EnableImageRecommend {
					recommenders = append(recommenders, m.RecommendImageBased)
				}
			case "latest":
